...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (hasErrors(errors)) {
      ...
      setGlobalMessage("يرجى تصحيح الأخطاء أدناه.");
      return;
    }
...
الرسالة: ${message || "لا توجد"}`;
    ...
    try {
      ...
      setStatus("success");
      setGlobalMessage(
        "تم استلام الطلب. سأعود إليك خلال 48 ساعة."
      );
      setName("");
      setEmail("");
      setTrainingLevel("");
      ...
      <Section id="contact" eyebrow={contact.eyebrow} dark>
        <div className="text-center">
          ...
          <div className="border border-lime bg-forest p-8 max-w-lg mx-auto">
            <p className="text-lime font-bold text-lg mb-2">تم استلام الطلب</p>
            <p className="text-neutral-light text-sm">{globalMessage}</p>
          </div>
        </div>
      </Section>
    );
  ...
  return (
    <Section id="contact" eyebrow={contact.eyebrow} dark>
        <div ref={refHeading} className={animHeading}>
          ...
          <p className="text-neutral-light text-base leading-relaxed mb-8 max-w-xl">
            {contact.subtitle}
          </p>
        </div>
